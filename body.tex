...
This provide  us with the notion of {\bf Inverse Feature Term Frequency}  (IFTF)  (the word `term'  is to emphasise that features are words),   as follows:
...
Then define {\bf Term Frequency-Inverse Target Term Frequency} (ITTF) as follows:
...
TF can also be dualized from documents to features and to target terms to obtain the corresponding expressions in each computation formula.  We refer to these by Feature Term Frequency and Target Term  Frequency. The word `term'  is kept and the adjectives `feature' and `target' are added to it so we can distinguish them. In IR, it is `within document term frequency', for which we can also use a location notation as follow
\[
...
\begin{quote}
 \ref{ftf-iftf} =  \ref{ttf-iftf} \qquad \quad  {\bf FTF-IFTF}  = {\bf TTF-IFTF} \\
\ref{ftf-ittf} =  \ref{ttf-ittf}  \qquad {\bf FTF-ITTF}  = {\bf TTF-ITTF} 
\end{quote}


But which one to use and which one has in fact been used in DS? In the next section we show that {\bf TTF-ITTF} can be used to get DS's {\bf PMI}. 
