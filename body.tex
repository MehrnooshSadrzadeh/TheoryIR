...
\section{Duality Considerations}

\subsection{IR: Term-Document Matrix}

document vector: vector component $d_t$: $\TF(t,d) \cdot \IDF(t)$.

term vector: vector component $t_d$: $\TF(t,d) \cdot \ITF(d)$.
...
\ITF(d) := -\log P_T(d)
\]
$\ITF$ is large for documents that contain few distinct terms.
Note that $T$ is the set of terms.
Multiple occurrences of the same term in a document are not considered.

\subsection{NLP: TargetTerm-FeatureTerm Matrix}
...
document-document similarity corresponds to target-target similarity

term-term similarity corresponds to target-target similarity
...
COMMENT: the IFF is the same for each feature in the target vector.

Rewriting:
\[
IFF(t,F) \cdot \vec{t}
\]
Means we get a product of scalar and target vector?

Therefore, I added section on dualities.
For the target, the vector component should be based on inverse target frequency(feature)?
